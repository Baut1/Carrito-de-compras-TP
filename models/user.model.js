...
function getPermisosByUserId (userId) {
  const result = db.prepare(`
    SELECT permisos.id, permisos.nombre
    FROM permisos
    JOIN rol_permiso ON permisos.id = rol_permiso.permiso_id
    JOIN users ON users.role_id = rol_permiso.rol_id
    WHERE users.id = ?
  `).all(userId)
  return result;
}

module.exports = { getAll, getById, create, update, softDelete, getPermisosByUserId };